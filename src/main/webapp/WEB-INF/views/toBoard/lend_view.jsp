...
<head>
    ...
    $(function(){
		...
		$("#deleteBtn").on("click",function () { //게시글 삭제
					...
					if (check) {
						location.href = "/lendBoard/lendDelete?seq=" + $("#deleteBtn").val(); //게시글 삭제 확인 팝업
					} else {
						return;
					}
				});
		...
		$("#modifyBtn").on("click",function () { //게시글 수정
					...
					if (check) {
						location.href = "/lendBoard/modify?seq=" + $("#modifyBtn").val(); //게시글 수정 확인 팝업
					} else {
						return;
					}
				});
...
<%-- 						</c:forEach>
 --%>							</div>
				...
				<section id="user-profile">
					<div id="nickname_box">
						<h2 id="nickname">
							<a href="/profile/userProfile?id=${board.writer}"><b>${writerInfo.name}</b>님</a>
						</h2>
