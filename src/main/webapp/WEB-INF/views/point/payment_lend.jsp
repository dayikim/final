										...
										.done(
												function(resp) {
													...
													if (resp == 0) {
														...
														$(".alertPoint")
																.text(
																		"보유 포인트 이상 사용은 불가능 합니다.");
														$(".alertPoint").css(
																"color", "red");
														$(".alertPoint").css(
																"font-weight",
																"800");
														alert("사용포인트가 결제금액보다 적습니다.\n충전하고 다시 결제해주세요.\n바로 충전페이지로 이동합니다.")
			...
			<div class="form-control wrapper">
				...
				<section id="lend_product">
					...
					<div class="lend_period">
						...
						<c:forEach var="board" items="${board }">
							...
							<span class="align-baseline" id="price"><b
								class="final_price">${board.price}</b> 상추</span>
							<div id="profile-image">
								<c:choose>
									<c:when test="${board.sysName != null }">
										<div class="image">
											<a
												href="/lendBoard/lendView?seq=${board.seq}&id=${board.writer}"><img
												src="/imgs/lend/${board.sysName }"></a>
										</div>
									</c:when>
									<c:otherwise>
										<div class="image">
				...
				<section id="payment">
					<div class="point_wrap">
						...
						<span id="use_point"><b id="use">사용 포인트 : </b> <input
							class="amount" name="point"> <b id="point2">상추</b> <input
							type="hidden" value="${point}" id="pointAmount">
				...
				<div class="btn_wrap">
					<form action="/point/payment" id="payForm">
						...
						<input type="hidden" name="item" value="${board.title}"> <input
							type="hidden" name="price" value="${board.price}" id="price">
						<input type="hidden" name="parentseq" value="${board.seq}"
							id="parentseq"> <input type="hidden" name="category"
							value="${board.category}" id="category"> <input
							type="hidden" name="paymentable" value="y">
